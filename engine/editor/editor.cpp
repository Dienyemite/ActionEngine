...
void Editor::SetScriptSystem(ScriptSystem* scripts) {
    ...
    if (m_inspector_panel) {
        ...
        m_inspector_panel->SetCreateScriptCallback([this](const std::string& name) {
...
// ============================================================================
// CreateScriptFile — generate a C++ script template and open it in VS Code
// ============================================================================

void Editor::CreateScriptFile(const std::string& script_name) {
    namespace fs = std::filesystem;

    // Validate: letters, digits, underscores only (prevent shell injection)
    for (unsigned char c : script_name) {
        if (!std::isalnum(c) && c != '_') {
            Log("Invalid script name. Use letters, digits, and underscores only.", 2);
            return;
        }
    }

    // Ensure game/scripts/ directory exists
    const fs::path scripts_dir = "game/scripts";
    std::error_code ec;
    fs::create_directories(scripts_dir, ec);
    if (ec) {
        Log("Failed to create scripts directory: " + ec.message(), 2);
        return;
    }

    // Write the script header from template (skip if already exists)
    const fs::path script_path = scripts_dir / (script_name + ".h");
    if (!fs::exists(script_path)) {
        std::ofstream f(script_path);
        if (!f) {
            Log("Failed to write script file.", 2);
            return;
        }
        f << "#pragma once\n"
          ...
          << ")\n"
             ...
             "        // Called once when this script starts\n"
             "    }\n"
             "\n"
             "    void OnUpdate(float dt) override {\n"
             "        // Called every frame (dt = delta time in seconds)\n"
             "        // Helpers: GetInput(), GetPosition(), GetComponent<T>(), Log()\n"
             "    }\n"
             "\n"
    ...
    // Collect all user scripts and regenerate register_user_scripts.h
    std::vector<std::string> names;
    for (const auto& entry : fs::directory_iterator(scripts_dir, ec)) {
        if (ec) break;
        const auto ext  = entry.path().extension().string();
        const auto stem = entry.path().stem().string();
        if (ext == ".h" && stem != "register_user_scripts") {
    ...
    // Use an absolute native path so the command works regardless of CWD.
    std::string abs_path = std::filesystem::absolute(script_path).string();
    LOG_INFO("[Editor] Created script '{}' — rebuild the project to register it.", script_name);
    Log("Script created: " + script_name + ".h  —  rebuild project to register.", 0);

#ifdef _WIN32
    {
        // Use cmd.exe /c so that 'code' (a .cmd batch file) is resolved from the
        // user's PATH. -n forces VS Code to open in a new window.
        std::string cmd = "cmd.exe /c code -n \"" + abs_path + "\"";
        ...
        if (CreateProcessA(nullptr, cmd_buf.data(), nullptr, nullptr,
            ...
            Log("Could not launch VS Code. Open manually: " + abs_path, 1);
        }
    }
#else
    std::system(("code -n \"" + abs_path + "\" &").c_str());
#endif
}
